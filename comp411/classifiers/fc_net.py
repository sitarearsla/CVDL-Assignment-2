...
class ThreeLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=(64, 32), num_classes=10,
        ...
        self.params["b1"] = np.zeros(hidden_dim[0], dtype=float)
        self.params["b2"] = np.zeros(hidden_dim[1], dtype=float)
    ...
    def loss(self, X, y=None):
        ...
        out_1, cache_1 = affine_lrelu_forward(X, W1, b1, lrelu_param={'alpha': self.alpha})
        out_2, cache_2 = affine_lrelu_forward(out_1, W2, b2, lrelu_param={'alpha': self.alpha})
        ...
        dout_3, grads["W3"], grads["b3"] = affine_backward(d_scores, cache_3)
        dout_2, grads["W2"], grads["b2"] = affine_lrelu_backward(dout_3, cache_2)
        dout_1, grads["W1"], grads["b1"] = affine_lrelu_backward(dout_2, cache_1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for layer in range(self.num_layers):
            if layer != max(range(self.num_layers)):
                output_dims = hidden_dims[layer]
            else:
                output_dims = num_classes
            self.params["b" + str(layer+1)] = np.zeros(output_dims)
            self.params["W" + str(layer+1)] = np.random.normal(0.0, weight_scale, (input_dim, output_dims))
    ...
    def loss(self, X, y=None):
        ...
        output, cache, dropout_cache = X, {}, {}
        for layer in range(self.num_layers):
            if layer == max(range(self.num_layers)):
                output, cache[layer] = affine_forward(output, 
                                                      self.params["W" + str(layer+1)], 
                                                      self.params["b" + str(layer+1)])
            else:
                output, cache[layer] = affine_lrelu_forward(output, 
                                                            self.params["W" + str(layer+1)], 
                                                            self.params["b" + str(layer+1)], 
                                                            {"alpha": self.alpha})
                if self.use_dropout:
                    output, dropout_cache[layer] = dropout_forward(output, self.dropout_param)
        scores = output
        ...
        for layer in range(self.num_layers)[::-1]:
            W_layer = self.params["W" + str(layer+1)]
            loss = loss + 0.5 * self.reg * np.sum(W_layer**2)
            if layer != max(range(self.num_layers)):
                if self.use_dropout:
                    dout = dropout_backward(dout, dropout_cache[layer])
                dout, dW, db = affine_lrelu_backward(dout, cache[layer])
            else:
                dout, dW, db = affine_backward(dout, cache[layer])
            grads["b" + str(layer+1)] = db
            grads["W" + str(layer+1)] = dW + self.reg * W_layer
