...
def affine_forward(x, w, b):
    ...
    output = np.dot(x.reshape(x.shape[0], -1), w) 
    out = output + b
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H_spatial = np.int(1 + (H + 2 * pad - HH) / stride)
    W_spatial = np.int(1 + (W + 2 * pad - WW) / stride)
    output = np.zeros([N, F, H_spatial, W_spatial])
    padded = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)), 'constant')
    
    for n in range(N):
        for h in range(H_spatial):
            for w_s in range(W_spatial):
                x_padded = padded[n ,: ,h*stride:h*stride+HH, w_s*stride:w_s*stride+WW]
                for f in range(F):
                    x_padded_sum = np.sum(x_padded * w[f,])
                    output[n, f, h, w_s]=  x_padded_sum + b[f]
    out = output
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_spatial = np.int(1 + (H - pool_height) / stride)
    W_spatial = np.int(1 + (W - pool_width) / stride)
    output = np.zeros((N, C, H_spatial, W_spatial))
    
    for n in range(N):
        for c in range(C):
            for h in range(H_spatial):
                for w in range(W_spatial):
                    output[n, c, h, w] = np.max(x[n, c, h * stride:h * stride + pool_height, w * stride:w * stride + pool_width])
    out = output
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for h in range(H_spatial):
                for w in range(W_spatial):
                    max_pool = x[n, c, h * stride:h * stride + pool_height, w * stride:w * stride + pool_width]
                    dx[n, c, h * stride:h * stride + pool_height, 
                       w * stride:w * stride + pool_width] += (max_pool == np.max(max_pool)) * dout[n, c, h, w]
